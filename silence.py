...
class FitnessFunction:
    ...
    def evaluate_fitness(self, midi_sequence_file):
        if FLAGS.save_generator_bundle:
            ...
            print(len(extracted_melodies[0]))
...
for i in range(10):
    ...
    for j in range(100):
        if j < i*10:
            ...
            s.append(music21.note.Note())
        else:
            s.append(music21.note.Note(type='half'))

    mf = music21.midi.translate.streamToMidiFile(s)
    mf.open('.temp.mid', 'wb')
    mf.write()
    mf.close()
    print('log likelihood with %i/100 rests'%(i*10),ff.evaluate_fitness('.temp.mid'))
